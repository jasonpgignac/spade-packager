...
describe "bpm unpack" do
  ...
  it "builds a gem from a given package.json" do
    ...
    bpm "package", "unpack", "coffee-1.0.1.pre.spd"

    exit_status.should be_success
    output = stdout.read
    output.should include("Unpacked bpm package into: #{Dir.pwd}/coffee-1.0.1.pre")
  ...
  it "can unpack to a different directory" do
    FileUtils.cp fixtures("coffee-1.0.1.pre.spd"), "."
    bpm "package", "unpack", "coffee-1.0.1.pre.spd", "--target", "star/bucks"

    exit_status.should be_success
    output = stdout.read
    output.should include("Unpacked bpm package into: #{Dir.pwd}/star/bucks/coffee-1.0.1.pre")

    home("star/bucks/coffee-1.0.1.pre/bin/coffee").should exist
  end

  it "can unpack more than one bpm package" do
    FileUtils.cp fixtures("coffee-1.0.1.pre.spd"), "."
    FileUtils.cp fixtures("jquery-1.4.3.spd"), "."
    bpm "package", "unpack", "coffee-1.0.1.pre.spd", "jquery-1.4.3.spd"

    exit_status.should be_success
    output = stdout.read
    output.should include("Unpacked bpm package into: #{Dir.pwd}/coffee-1.0.1.pre")
    output.should include("Unpacked bpm package into: #{Dir.pwd}/jquery-1.4.3")
  ...
  it "shows a friendly error message if bpm can't write to the given directory" do
    ...
    bpm "package", "unpack", "jquery-1.4.3.spd", :track_stderr => true
